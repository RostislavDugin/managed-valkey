...
Корневой `just run` готовит dev-базу и k3s, выполняет bootstrap и миграции, затем запускает оператор, API и Vite. `just down` останавливает dev-инфраструктуру без удаления данных. Корневой `just test` параллельно запускает `just test` в `api`, `operator`, `web` и `tests/e2e`, ждёт каждый набор и сохраняет отдельные журналы. `operator just test` выполняет полный локальный набор, а `operator just test-ci` запускает только unit/envtest и отмечает отсутствие k3s-матрицы в отчёте.

[Команды сборки, миграций и тестов](SYSTEM-DETAIL.md#justfile).

### CI

Задание `test-operator` вызывает `operator just test-ci`. Оно выполняет unit и
...
Каждый запрос на слияние и `push` проходит последовательные этапы `lint`, `test` и `build`. Внутри первых двух этапов API, оператор и клиентское приложение проверяются параллельно. CI/CD выполняет явно определённый ограниченный набор тестов. Полные ресурсоёмкие наборы, включая многокластерные сценарии отказов оператора, запускаем локально; в CI/CD они не входят. Локальный `just test` сохраняет полную обязательную матрицу. API получает однонодовый k3s и отдельную базу; проверки оператора в CI включают unit и envtest. Затем параллельно собираются образы API с миграциями и Caddy со SPA для `linux/amd64`. Успешный `push` в `main` запускает задание `deploy` через окружение GitHub `production`; запрос на слияние сервер не меняет.
...
k3s в compose даёт многонодовый стенд с тем же anti-affinity, что в рабочем окружении. В среде разработки оператор запускается на хосте и работает с Kubernetes API через отдельный kubeconfig. Bootstrap добавляет точные маршруты от хоста к Pod CIDR выбранного кластера. API и оператор используют отдельные тестовые k3s при каждом полном локальном запуске. Полный ресурсоёмкий набор оператора запускается локально; задание CI `test-operator` выполняет `operator just test-ci` без k3s.
