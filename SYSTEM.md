...
Внутренняя цель доступности `ha` составляет 99,95% в месяц, около 22 минут простоя. Это ориентир для разработки, а не договорное обязательство. Внешний монитор проверяет DNS, балансировщик, TLS и Valkey, но сервис не рассчитывает месячный процент доступности как договорный показатель.
...
`api` и оператор записывают ошибки своих операций в VictoriaLogs. Публичный `/health` проверяет PostgreSQL, доступ API к Kubernetes, свежесть наблюдений и незавершённые операции. Создание, изменение или удаление старше пяти минут считается застрявшим. Фазы `error` и `unavailable`, а также признаки восстановления дают ошибку; `degraded` остаётся предупреждением с HTTP 200.

Публичный `/valkey-health` принимает URI проверочного инстанса в заголовках, открывает пользовательское TLS-подключение, выполняет AUTH и PING. URI и пароль не попадают в ответ или логи. Uptime Kuma опрашивает оба маршрута каждые 30 секунд и уведомляет после шести неудачных повторов, поэтому краткий сетевой сбой остаётся только в истории проверок.
...
Постоянный внешний монитор использует `/health` как общий сигнал о платформе. Отдельный монитор `/valkey-health` проверяет primary и, для HA, read-only адрес выбранного инстанса. Если у него включён whitelist, исходящий адрес виртуальной машины с API должен входить в разрешённые сети. Пароли задаются только в секретных заголовках монитора.
