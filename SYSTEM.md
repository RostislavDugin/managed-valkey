...
Каждый запрос на слияние и `push` сразу создаёт десять независимых заданий:
три lint, быстрые тесты API, оператора и web, общую интеграцию, полный Playwright
e2e и две сборки образов. API использует отдельную базу без k3s, оператор
запускает unit/envtest, а web запускает Vitest. Общая интеграция готовит
однонодовый k3s. Job полного e2e создаёт три параллельных задания матрицы для
жизненных циклов, квот и отказов; каждая получает отдельный трёхнодовый стенд.
Сборки API с миграциями и Caddy со SPA выпускают образы для `linux/amd64`
независимо от результатов проверок.

Итоговое задание дожидается всех десяти результатов и принимает только
