...
| CR → объекты k8s | оператор | `spec`, Secret, поды, ноды, шлюз и `INFO` для проверки состояния | Объекты инстанса, служебные поля Secret, `status` CR и Events | События и `RequeueAfter`; здоровье и операции каждую 1 с, heartbeat каждые 10 с |
