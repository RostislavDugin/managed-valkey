...
| `NOT_ENOUGH_RESOURCES`  | 422  | Превышена квота кластера или резервы нельзя распределить по нодам; тот же код стоит в причине фазы       |
...
| `UNAVAILABLE`           | 503  | Истёк срок ожидания блокировки или точной проверки размещения                                            |
...
У `NOT_ENOUGH_RESOURCES` поле `details.reason` различает общий дефицит `cluster_quota` и невозможную раскладку `placement_capacity`. При невозможной раскладке ответ содержит число нод, размер процесса и доступный бюджет ноды, но не раскрывает чужие инстансы или имена нод. Причина `placement_check_timeout` относится к `503 UNAVAILABLE`. Значение `phase_reason` с кодом `NOT_ENOUGH_RESOURCES` по-прежнему означает, что уже созданный `Pod` ожидает размещения.
...
| Managed Kubernetes | пять переменных `MANAGED_K8S_NODE_*` | 3 ноды по 4 vCPU / 16 GiB, резерв каждой 1000m / 2048 MiB |


Максимальный размер инстанса и бюджет Managed Kubernetes отвечают за разные ограничения. Первый ограничивает размер одного процесса Valkey. `MANAGED_K8S_NODE_COUNT`, `MANAGED_K8S_NODE_CAPACITY_VCPU` и `MANAGED_K8S_NODE_CAPACITY_RAM_GB` описывают одинаковые физические ноды. `MANAGED_K8S_NODE_RESERVED_CPU_MILLI` и `MANAGED_K8S_NODE_RESERVED_RAM_MIB` вычитаются из каждой ноды. В рабочей среде Valkey получает по 3000m CPU / 14336 MiB на ноду, всего 9000m / 43008 MiB. Это настроенный резерв сервиса, а не текущее значение Kubernetes `allocatable` и не показания загрузки нод. Ограничение в 32 инстанса следует из числа `listener` одного `Gateway` и не требует отдельной переменной окружения.
...
Ресурсные квоты, предел 32 инстанса и размещение по нодам проверяются при создании и изменении размера внутри одной транзакции под общей блокировкой `pg_advisory_xact_lock(1)` (раздел 5). Точная проверка одновременно учитывает CPU и RAM. Один процесс `single` должен помещаться на одной ноде, три процесса `ha` должны помещаться на разных нодах. Отказ не создаёт инстанс, аудит, биллинговый период или запись ключа. При изменении размера собственный резерв заменяется кандидатом без двойного учёта. Если лимит вручную снизили ниже использования, изменение допустимо по каждой размерности, когда оно не увеличивает прежний резерв.

`GET /v1/managed/valkey/capacity` одним запросом PostgreSQL возвращает личный лимит и расход, суммарный доступный после инфраструктурного резерва бюджет нод и расход всех владельцев, а также предел и число активных инстансов. Маршрут не обращается к Kubernetes и не берёт общую блокировку. Страница создания и окно изменения тарифа запрашивают этот снимок сразу, затем примерно раз в пять секунд. Следующий запрос начинается только после завершения предыдущего. Личная квота имеет приоритет над общей ёмкостью, а предел инстансов применяется только к созданию. Снимок проверяет суммарный бюджет, но не моделирует размещение по нодам. Окончательное решение принимает проверка внутри изменяющего запроса.

Проверка мутации доказывает существование расчётной раскладки, но не назначает `Pod` конкретным нодам и не учитывает внезапный отказ ноды. Фактическое размещение остаётся за Kubernetes.
...
необходимой топологией: одна нода для обычных проверок `single`, две для переноса
`single` после потери worker или разнесённых Envoy, три для HA и четыре для HA
с запасной нодой. Число нод включает server; конкретный сценарий может требовать
другого размещения. Unit и envtest не требуют k3s. Число одновременно работающих
