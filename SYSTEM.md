...
В change `add-valkey-management-api` реализуем HTTP API управления инстансами, PostgreSQL-модели, квоты, внутренний сервис аудита, запись биллинговых периодов и подключение фронта. Фоновую синхронизацию, Kubernetes/k3s-тесты, HTTP API аудита и метрик, а также расчёт месячного расхода делаем позже. Приведённые далее описания этих компонентов задают целевую архитектуру и не расширяют текущую задачу.
