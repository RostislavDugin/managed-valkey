...
Образы и инструменты закрепляем конкретными версиями или digest, `latest` не используем. Изменение комбинации проходит интеграционную проверку перед развёртыванием. Для остановки и замены Valkey обязателен `ContainerRestartRules`; сценарий выпуска проверяет его на каждой рабочей ноде. Текущий кластер `v1.34.1` перезапускает проверочный контейнер и пока не допускается к развёртыванию Valkey.
