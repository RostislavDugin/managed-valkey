...
Здесь идемпотентность означает, что повтор того же запроса с тем же `Idempotency-Key` не выполняет изменение второй раз. Это относится к созданию инстанса, ресайзу и смене пароля. Это нужно, когда API уже сохранил изменение, но клиент не получил ответ из-за обрыва соединения. Клиент повторяет запрос, а API узнаёт ранее выполненную операцию и возвращает её прежний результат.
