...
Максимальный размер инстанса и бюджет Managed Kubernetes отвечают за разные ограничения. Первый ограничивает размер одного процесса Valkey. `MANAGED_K8S_NODE_COUNT`, `MANAGED_K8S_NODE_CAPACITY_VCPU` и `MANAGED_K8S_NODE_CAPACITY_RAM_GB` описывают одинаковые физические ноды. `MANAGED_K8S_NODE_RESERVED_CPU_MILLI` и `MANAGED_K8S_NODE_RESERVED_RAM_MIB` вычитаются из каждой ноды. В рабочей среде Valkey получает по 3000m CPU / 14336 MiB на ноду, всего 9000m / 43008 MiB. Ограничение в 32 инстанса следует из числа `listener` одного `Gateway` и не требует отдельной переменной окружения.
...
Проверка доказывает существование расчётной раскладки, но не назначает `Pod` конкретным нодам и не учитывает внезапный отказ ноды. Фактическое размещение остаётся за Kubernetes.
