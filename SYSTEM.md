...
За создание и изменение настроек отвечают две наши подсистемы. `api` принимает запросы из веб-интерфейса и сохраняет выбранные пользователем настройки в PostgreSQL. Фоновые циклы внутри `api` доставляют их через Kubernetes API в ресурс `ValkeyInstance` и переносят его состояние и метрики обратно в базу.
...
| ---------- | ------------------------------------------------------------------------------------------------------------------------------------ | ---------------------- | -------------------------------------------- |
| `api` | Пользователи, квоты, биллинг, передача выбранных пользователем настроек в CR, сохранение статусов и истории метрик в PostgreSQL, диагностика, статика SPA | Go, gin, gorm, client-go | Виртуалка, compose |
| `operator` | Приводит состояние k8s к заданному в CR, выполняет failover и перекатку, записывает состояние и последние метрики в `status` | Go, controller-runtime | Внутри k8s; на dev как процесс хоста |
...
flowchart LR
    ...
    api -- "логи" --> vl
    operator -- "OTLP, HTTPS и Basic Auth" --> caddy
    browser -- "vmui, HTTPS и Basic Auth" --> caddy
    caddy -- "просмотр логов и OTLP оператора" --> vl
    client -- "TLS на VALKEY_PUBLIC_PORT" --> gw
    gw -- "по SNI в Service инстанса" --> pods
```

`api` принимает запрос на создание инстанса и возвращает HTTP `202` со статусом `provisioning`: запрос сохранён, запуск ещё не завершён. Фоновые циклы API передают выбранные настройки в CR `ValkeyInstance`, а состояние, которое проверил оператор, переносят из `status` в PostgreSQL. Оператор по CR создаёт и настраивает объекты Kubernetes. Три цикла описаны в разделе 6.

`api` также отдаёт собранные файлы SPA. Интерфейс `web` получает статусы и историю метрик через API из PostgreSQL.
...
| браузер эксплуатации | Caddy, дальше VictoriaLogs vmui | HTTPS 443, `logs.h3llo-demo.com` | отдельная учётная запись `logs-reader`, Basic Auth |
...
| оператор | Caddy, дальше VictoriaLogs на VM | OTLP/HTTPS 443, `logs.h3llo-demo.com` | отдельная учётная запись `valkey-operator`, Basic Auth, только запись OTLP |
...
Публичные запросы к API и логам принимает Caddy по HTTPS 443; у шлюза открыт `VALKEY_PUBLIC_PORT`. PostgreSQL на 45432 и VictoriaLogs на 9428 недоступны из интернета. API обращается к ним по внутренней сети compose. Для отладки PostgreSQL публикуется только на `127.0.0.1` VM; удалённое подключение идёт через SSH-туннель и требует пароля БД. TLS PostgreSQL внутри compose отключён. На VM монтируется kubeconfig отдельной учётной записи для публичного endpoint Managed Kubernetes. Его адрес, порт, доверенный CA и доступ с VM проверяются на стенде. Приватной сети и VPN между VM и кластером нет. Оператор отправляет логи через Caddy по HTTPS с проверкой сертификата и отдельной авторизацией; недоступность этого пути останавливает только экспорт логов.
...
`desired_generation` хранит номер принятой конфигурации, `observed_generation` — номер полностью применённой. Такой номер называем поколением. Оно растёт при ресайзе, изменении whitelist или пароля. Пока номера отличаются, интерфейс показывает «Применяется», а API не принимает следующее изменение конфигурации. Имя и maintenance поколение не меняют. Версии `password_version` и `applied_password_version` отдельно отмечают принятие и завершение смены пароля.

Оператор подтверждает применение после проверки фактического результата. Одна запись настроек в CR ещё ничего не подтверждает. До окончания ресайза база хранит и выбранный, и фактически применённый размер; квоту считаем по большему из них. Удаляемый инстанс занимает квоту до подтверждения освобождения ресурсов.
...
История биллинга хранит периоды с зафиксированной ценой: новый тариф действует с принятия ресайза, тарификация заканчивается с запроса удаления. История фаз помогает разбирать инциденты, но не измеряет доступность для клиента. Аудит отдельно фиксирует автора и принятое действие. Метрики хранятся 7 дней, ответы для идемпотентных повторов — 24 часа.
...
| Kubernetes не может разместить под primary уже созданного инстанса, в том числе `single` при ресайзе                   | `NOT_ENOUGH_RESOURCES` | `unavailable`                                                                                                                         |
...
Здесь идемпотентность означает, что повтор того же запроса с тем же `Idempotency-Key` не выполняет изменение второй раз. Это относится к регистрации, созданию инстанса, ресайзу и смене пароля. Это нужно, когда API уже сохранил изменение, но клиент не получил ответ из-за обрыва соединения.
...
Для создания инстанса, ресайза и смены пароля `idempotency_keys` хранит ключ, отпечаток запроса (метод, путь, тело) и ответ 24 часа. Повтор с тем же ключом и отпечатком возвращает сохранённый ответ. Тот же ключ с другим отпечатком получает `422 IDEMPOTENCY_MISMATCH`. Ключ проверяется и записывается в одной транзакции с изменением. Авторизация и проверка владельца идут до поиска ответа; найденный успешный повтор возвращается до проверки текущей фазы и занятой операции. Иначе принятый ресайз сам заблокировал бы свой повтор.
...
Поколение конфигурации и версия пароля описаны в [разделе о данных](#настройки-и-наблюдаемое-состояние). При смене пароля растут оба счётчика; при ресайзе и изменении whitelist растёт только поколение. По `configuration_requested_at` интерфейс и диагностика определяют, сколько длится применение настроек.
...
UI генерирует пароль и отправляет запрос с `Idempotency-Key`. API проверяет параметры и квоты, затем одной транзакцией сохраняет инстанс, открывает биллинговый период и запоминает ответ для повторов. Полный пароль на сервере не сохраняется.
...
В API работают два независимых фоновых цикла с `config.SyncInterval=5s`. Один передаёт выбранные настройки из базы в Kubernetes, второй сохраняет в базу результаты проверок оператора. В v1 API запускается одной репликой; проходы одного цикла не перекрываются. Ошибка одного инстанса не останавливает обработку остальных. После перезапуска циклы продолжают работу по данным PostgreSQL и Kubernetes, отдельной очереди в памяти не требуется.

Только активный оператор, владеющий Lease, запускает сбор метрик и цикл приведения фактического состояния к заданному (reconcile). Для одного инстанса проходы reconcile выполняются последовательно; разные инстансы могут обрабатываться параллельно.

| Цикл | Где | Читает | Пишет | Период |
| ---- | --- | ------ | ----- | ------ |
| База → CR | `api` | `valkey_instances`, текущие CR и Secret | Namespace, хеши в Secret, `spec` CR; после удаления `deleted_at` и событие `deleted` в базе | `config.SyncInterval`, 5 с |
...
| CR → база | `api` | `status` CR, включая `nodes` и `metrics` | Наблюдаемые колонки, ноды, события обнаруженных фаз, история метрик | `config.SyncInterval`, 5 с |
...
CR связывает API и оператор. В `spec` API записывает настройки, версию пароля и поколение конфигурации. В `status` оператор сохраняет фактическое состояние, применённые версии, результаты проверки сети и последние снимки метрик. Паролей и хешей в CR нет.
...
Конфигурация привязана к размеру и образу конкретного пода. При ресайзе старая конфигурация сохраняется до замены использующих её подов, чтобы их случайный рестарт не загрузил настройки другого размера.
...
Раз в `config.MetricsInterval`, равный 10 секундам, оператор собирает данные каждого пода: `INFO` через клиент `valkey-go` (server, memory, clients, stats, replication) под пользователем `operator` и CPU из `metrics.k8s.io`. Для CPU нужен metrics-server: в k3s он есть, для кластера h3llo это проверка в разделе 15. Оператор сохраняет результат в `status.metrics` CR своего инстанса. API не подключается к Pod IP и не вызывает `INFO`.

`status.metrics` содержит только последний успешный снимок каждого пода по его номеру в StatefulSet (ordinal): максимум один для `single` и три для `ha`, без истории. CRD ограничивает список тремя элементами с уникальным ordinal. Каждый снимок содержит Pod UID, container ID, `runId`, `collectedAt` в UTC с точностью до микросекунд, роль процесса и поля метрик из [схемы CR](SYSTEM-DETAIL.md#cr-valkeyinstance). `collectedAt` фиксируется при сборе, после успешного `INFO`; повторная запись или чтение CR его не меняет. Оператор проверяет идентичность процесса до и после сбора. Если за время сбора процесс сменился, оператор отбрасывает снимок. Метрики нового процесса появятся только после успешного сбора именно с него.
...
На виртуалке перед `api` стоит Caddy. Он получает и продлевает сертификат Let's Encrypt для `app.h3llo-demo.com` и ограничивает частоту запросов. Статику отдаёт сам `api`.
...
Для логов Caddy обслуживает отдельный HTTPS-домен `logs.h3llo-demo.com`. У оператора есть учётная запись только для экспорта, у эксплуатации — отдельная для просмотра. Остальные пути VictoriaLogs снаружи закрыты; доступ без настроенной авторизации запрещён.

[Caddyfile, сборка плагина ограничения запросов и авторизация логов](SYSTEM-DETAIL.md#caddy-перед-api).
...
Правило: `ram_gb >= vcpu` и `ram_gb <= 16 * vcpu`. Интерфейс показывает все допустимые сочетания из сетки. `VALKEY_INSTANCE_MAX_VCPU` и `VALKEY_INSTANCE_MAX_RAM_GB` ограничивают размер одного инстанса; `GET /v1/managed/valkey/sizes` отдаёт сетку в этих пределах вместе с ценами.
...
| Managed Kubernetes | env `MANAGED_K8S_NODE_VCPU`, `MANAGED_K8S_NODE_RAM_GB` | 24 vCPU и 48 GB RAM на ноду |


Максимальный размер инстанса и размер ноды Managed Kubernetes отвечают за разные ограничения. Первый проверяет запрошенный размер Valkey. Второй описывает место, на котором Kubernetes размещает процессы Valkey. Ограничение в 32 инстанса следует из числа listener одного `Gateway` и не требует отдельной переменной окружения.

Использование считается как сумма `nodes * vcpu` и `nodes * ram_gb` по строкам с `deleted_at is null`, где для каждой строки берётся большее из намерения и `applied_*` (раздел 4). Ресурсные квоты и предел 32 инстанса проверяются при создании и ресайзе внутри одной транзакции под общей блокировкой `pg_advisory_xact_lock(1)` (раздел 5).
...
API использует `http://victorialogs:9428/insert/opentelemetry/v1/logs` внутри compose. Оператор на проде отправляет логи в `https://logs.h3llo-demo.com/insert/opentelemetry/v1/logs` через Caddy с проверкой сертификата и Basic Auth под `valkey-operator`. Экспорт не проходит через API сервиса. Caddy разрешает этой учётной записи только `POST` на указанный путь; учётная запись просмотра `logs-reader` имеет доступ только к `/select/*`, включая vmui и API запросов. VictoriaLogs на 9428 доступен только внутри compose. [Приём OTLP](https://docs.victoriametrics.com/victorialogs/data-ingestion/opentelemetry/) и [защита VictoriaLogs](https://docs.victoriametrics.com/victorialogs/security-and-lb/).

Пароль экспорта генерирует команда эксплуатации и сохраняет в Secret `valkey-logs-export` в `valkey-system`. Secret монтируется в оператор только для чтения; `VL_OTLP_USERNAME` задаёт имя, `VL_OTLP_PASSWORD_FILE` путь к файлу пароля. Экспортёр формирует заголовок Basic Auth в памяти. Пароли не передаются в URL и аргументах процесса, заголовки и тела OTLP не логируются. Caddy хранит только bcrypt-хеши паролей экспорта и просмотра. При заданной авторизации экспорт разрешён только по HTTPS; ошибки TLS или `401` не вызывают повтор без авторизации или переход на HTTP.
...
| Карточка инстанса, вкладка «Настройки»   | Ресайз с новой ценой и предупреждением по режиму и типу изменения, окно maintenance, имя                                                                                                                                                                             |
...
На dev и в production используем одну закреплённую комбинацию версий инфраструктуры. Окружения запускаются вручную через отдельные compose-файлы; манифесты Kubernetes находятся в `deploy/dev` и `deploy/prod`.

### docker-compose.dev.yml

Dev compose запускает только PostgreSQL и локальный k3s из трёх нод. Скрипт `scripts/k3s_bootstrap.sh` на хосте устанавливает инфраструктуру Kubernetes и выдаёт приложениям отдельные kubeconfig. API и оператор работают на хосте через `just run` из своих каталогов; фронтенд запускается через Vite. Прямой доступ оператора к Pod IP нужно обеспечить перед реализацией команд Valkey.

[Состав dev-окружения, сеть, сборка и запуск](SYSTEM-DETAIL.md#docker-composedevyml).

### docker-compose.prod.yml

На VM работают Caddy, API, PostgreSQL и VictoriaLogs. База и хранилище логов не доступны напрямую извне. Оператор работает внутри managed Kubernetes h3llo вместе с Envoy Gateway и cert-manager; API подключается к Kubernetes с VM через отдельный kubeconfig.
...
Настройки подключения, тарифов, квот и таймаутов задаются через окружение соответствующего компонента. У оператора нет подключения к PostgreSQL. На production он получает права Kubernetes через ServiceAccount, а секрет для экспорта логов — из Secret.
...
Перед развёртыванием обязателен сценарий потери worker-ноды, в том числе с оператором и primary одновременно. Проверяем восстановление без ручной правки подов и CR, работу `single`, реплик и Envoy. При наличии запасной ноды должен восстановиться весь состав; без неё для `ha` допустим `degraded`.
...
Команды запуска и тестов Go-приложений выполняем через `just` из `api/` и `operator/`. Линтер и сборку запускает pre-commit из корня, фронтенд проверяем командами pnpm из `web/`.

[Команды сборки, миграций и тестов](SYSTEM-DETAIL.md#justfile).

### CI

Каждый push проходит линтеры, быстрые интеграционные тесты, envtest и проверки фронтенда. Полный e2e на k3s запускается ночью и вручную; перед развёртыванием обязателен успешный прогон именно разворачиваемой ревизии.

CI также проверяет код через CodeQL, зависимости и образы через Trivy, отдельно ищет секреты. Исправимые уязвимости HIGH/CRITICAL и найденные секреты блокируют сборку. Неисправленные уязвимости сохраняются в отчёте для разбора.
...
- Смена пароля, восстановление и ресайз продолжаются после рестартов и потерянных ответов без нарушения изоляции и подтверждений.
...
Фронтенд находится в `web/`, миграции PostgreSQL — в `migrations/`, манифесты Kubernetes — в `deploy/`. Статика фронтенда входит в образ API.
...
k3s в compose даёт многонодовый стенд с тем же anti-affinity, что в prod. На dev оператор запускается на хосте и работает с Kubernetes API через отдельный kubeconfig. Прямой доступ из хоста к Pod IP нужно добавить вместе с поведением оператора, которому он понадобится. Полный стенд дороже быстрых тестов, поэтому CI на каждый push использует PostgreSQL и envtest, а k3s запускается отдельно перед развёртыванием и по расписанию.

Тесты с настоящими PostgreSQL и kube-apiserver проверяют поведение транзакций, миграций и конфликтов. Fake-клиент нужен лишь там, где проверяется сопоставление данных с объектами; часы подменяются для проверки таймаутов. Go-команды лежат в `Justfile` каждого приложения, фронт запускается через pnpm из `web/`, окружение через compose из корня. CodeQL и Trivy подключаются в GitHub Actions.
