...
с одной или двумя репликами Envoy. API уже доставляет намерения в Kubernetes и
импортирует `status`, но совместный набор с настоящим оператором относится к
отдельному изменению. Сбор метрик оператором в этот этап не входит.
...
Dev сохраняет Compose-проект `managed-valkey-dev`, стабильные порты и тома. Каждый тестовый запуск получает отдельные Compose-проект, сеть, том, kubeconfig и localhost-порты. API, integration и e2e используют разные базы на общем локальном PostgreSQL; тестовая очистка не удаляет сервер или dev-базу. Playwright уже проверяет регистрацию без Valkey; общий integration-набор API с оператором и полные e2e-сценарии Valkey ещё не реализованы.
...
Каждый запрос на слияние и `push` проходит последовательные этапы `lint`, `test` и `build`. Внутри первых двух этапов API, оператор и клиентское приложение проверяются параллельно. CI/CD выполняет быстрые команды приложений: API использует отдельную базу без k3s, оператор запускает unit/envtest, web запускает Vitest. Полные ресурсоёмкие наборы выполняются локально через `just test-full` и в CI/CD не входят. Playwright остаётся отдельным заданием `test-e2e` и не входит в корневые `test` или `test-full`. Затем параллельно собираются образы API с миграциями и Caddy со SPA для `linux/amd64`. Успешный `push` в `main` запускает задание `deploy` через окружение GitHub `production`; запрос на слияние сервер не меняет.

CodeQL, Trivy и общие e2e-сценарии API с оператором пока не входят в этот процесс. Playwright проверяет регистрацию отдельным заданием `test-e2e`.
