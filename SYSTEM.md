...
Таблица `valkey_node_metrics`, HTTP API чтения, экран мониторинга, импорт снимков из CR и очистка истории уже работают. CRD принимает `status.metrics`, но оператор пока не собирает и не записывает эти снимки. HTTP API аудита и расчёт месячного расхода также остаются за границами этого этапа. Приведённые далее описания этих компонентов задают целевую архитектуру.
...
ограниченные отказы Kubernetes на отдельном трёхнодовом стенде. Сбор метрик
оператором в этот этап не входит.
