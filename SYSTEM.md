...
В API работают два независимых фоновых цикла с `SYNC_INTERVAL=5s`. Один передаёт выбранные настройки из базы в Kubernetes, второй сохраняет в базу результаты проверок оператора. В v1 API запускается одной репликой; проходы одного цикла не перекрываются. Ошибка одного инстанса не останавливает обработку остальных. После перезапуска циклы продолжают работу по данным PostgreSQL и Kubernetes, отдельной очереди в памяти не требуется.
...
На dev оба процесса compose используют внутренний `victorialogs:9428` без авторизации. Если оператор перенесён внутрь k3s, bootstrap задаёт доступный из кластера частный адрес контейнера VictoriaLogs в сети compose и проверяет отправку OTLP. Публикация dev-порта для браузера привязана к `127.0.0.1` хоста; открывать его в интернет для доступа из k3s не нужно. Недоступность экспортёра не блокирует HTTP, reconcile или метрики.
...
Compose запускает PostgreSQL, VictoriaLogs, API, оператор и локальный k3s из трёх нод. Bootstrap устанавливает инфраструктуру Kubernetes и выдаёт API и оператору отдельные права. Оператору нужен доступ к Pod IP; если его нельзя обеспечить узким сетевым правилом из compose, оператор запускается внутри k3s. Фронтенд работает на хосте через Vite.
...
Go-приложения собираем и проверяем командами `just` из корня репозитория, фронтенд — командами pnpm из `web/`.
