...
HTTP API управления инстансами, PostgreSQL-модели, квоты, внутренний сервис аудита, биллинговые периоды и веб-интерфейс управления уже работают. Фоновые процессы API доставляют настройки в Kubernetes и возвращают в PostgreSQL состояние инстанса, состав нод и историю фаз. Они начинают первый проход сразу после запуска, затем повторяют сверку раз в секунду. Тесты API проверяют этот обмен через отдельный однонодовый `k3s` без оператора. Общий интеграционный набор проверяет настоящий API и оператор вместе на отдельном однонодовом стенде.

Таблица `valkey_node_metrics`, HTTP API чтения, экран мониторинга, сбор снимков оператором, импорт из CR и очистка истории уже работают. HTTP API аудита и расчёт месячного расхода также остаются за границами этого этапа. Приведённые далее описания этих компонентов задают целевую архитектуру.

Изолированная реализация оператора уже поддерживает `single`, `ha`, переключение
primary после доказанной остановки, изменение размера и смену пароля. Свежие
наблюдения Valkey выполняются раз в секунду, а проверка Envoy остаётся отдельной
и выполняется раз в 10 секунд. Оператор проверяется напрямую через CR и Secret.
Полный локальный набор распределяет сценарии по независимым k3s на 1–4 нодах
с одной или двумя репликами Envoy. API доставляет намерения в Kubernetes и
импортирует `status`; совместную работу с настоящим оператором проверяет общий
интеграционный набор. Playwright проходит пользовательские жизненные циклы и
ограниченные отказы Kubernetes на отдельном трёхнодовом стенде.
...
| Публичный адрес с TLS | `<slug>.<VALKEY_BASE_DOMAIN>:<VALKEY_PUBLIC_PORT>` для primary и `<slug>-ro.<VALKEY_BASE_DOMAIN>:<VALKEY_PUBLIC_PORT>` для чтения. В `single` оба адреса ведут на единственный primary; в `ha` адрес `-ro` выбирает готовую реплику. Клиент обязан передавать SNI (раздел 7). |
...
| Квоты                 | На пользователя (по умолчанию 4 vCPU и 12 GB) и на весь кластер (через env).                                                                                                                                                                                |
...
- подтверждения почты, приглашений и защиты квот от регистрации нескольких аккаунтов. Регистрация открыта, квота 4 vCPU / 12 GB выдаётся сразу. При отсутствии оплаты несколько аккаунтов могут занять весь кластер.
...
Перед изменением инфраструктуры выпуск проверяет, что каждый агент Cilium сообщает `N/N reachable`, где `N` равно числу нод. В текущем однонодовом рабочем кластере ожидается `1/1 reachable`. Если кластер расширят, проверка автоматически потребует связность всех нод и конечных точек Cilium.
...
У `api` есть kubeconfig на dev. На проде фоновые процессы синхронизации можно выключить через `KUBERNETES_BACKGROUND_SYNC_ENABLED=false`, тогда API запускается без kubeconfig, но не доставляет изменения и не получает состояние или метрики из Kubernetes. HTTP-обработчики не вызывают Kubernetes: фоновые процессы используют отдельный клиент и повторяют неудачные операции. API получает следующие права:
...
| `secrets`, `configmaps`, `services`, `networkpolicies` | create, get, list, watch, update, patch, delete | Изменяемые объекты инстанса                                                     |
| `statefulsets`                                         | get, list, watch                                | Обнаружение пользовательских ресурсов прежней версии                            |
| `poddisruptionbudgets`                                 | create, get, list, watch, update, patch, delete | Ограничение добровольных выселений подов `ha`                                   |
| `pods`                                                 | create, get, list, watch, patch, delete         | Прямое создание, метки ролей и управляемая замена после подтверждения остановки |
...
В рабочем окружении оператор работает в `StatefulSet` внутри Kubernetes h3llo: одна реплика, `updateStrategy: OnDelete`, служебный `Service` без кластерного IP и выбор ведущего экземпляра. К конкретной рабочей ноде он не привязан. После падения процесса или обычного удаления `Pod` Kubernetes автоматически запускает оператор заново. После удаления рабочей ноды через ЛК он создаёт замену на другой ноде, когда облако удалит прежний объект `Node`. Здесь доверяем порядку: сначала уничтожение виртуальной машины, затем удаление `Node`. При потере связи с существующей нодой перенос может ждать её возврата или ручного подтверждения отключения. Принудительно удалять `Pod` оператора или объект `Node` при неподтверждённой остановке виртуальной машины запрещено. Обновление оператора также проходит с остановкой единственного `Pod`. Правила основаны на [ограничениях `StatefulSet`](https://kubernetes.io/docs/tasks/run-application/force-delete-stateful-set-pod/).
...
| Valkey вернул неожиданный ответ на служебную проверку                                                          | `PROCESS_RESPONSE_INVALID` | `unavailable` для primary; подробности ответа остаются в condition процесса                                                           |
...
| `GET`    | `/v1/managed/valkey/capacity`                          |                                                                                                                         | `{user: {limit, used}, cluster: {limit, used}, instances: {limit, used}}`                    |
...
```json
{
  "error": {
    "code": "QUOTA_EXCEEDED",
    "message": "...",
    "details": { "max_vcpu": 4, "requested": 6 }
  }
}
```
...
| `NOT_ENOUGH_RESOURCES`  | 422  | Превышена суммарная квота кластера; тот же код стоит в причине фазы                                      |
...
| `UNAVAILABLE`           | 503  | Истёк срок ожидания блокировки либо недоступна внешняя зависимость                                       |
...
У `NOT_ENOUGH_RESOURCES` поле `details.reason` различает личную квоту, общий дефицит `cluster_quota` и предел числа инстансов. Значение `phase_reason` с кодом `NOT_ENOUGH_RESOURCES` означает, что уже принятый `Pod` ожидает размещения по обязательным правилам Kubernetes.
...
Под общей блокировкой `api` проверяет квоты с учётом `max(новый размер, applied_*)` для изменяемого инстанса и `max(desired, applied)` для остальных, закрывает текущий биллинговый период и открывает новый, меняет размер и увеличивает `desired_generation`. Оператор фиксирует целевое поколение и размер в `status.rollout` до замены первого `Pod`. `applied_*` меняется только после полной перекатки. Следующее изменение размера станет доступно после записи подтверждения в PostgreSQL.
...
Служебное пространство имён `valkey-system` (переменная окружения `VALKEY_SYSTEM_NAMESPACE`) содержит оператор, общий `Gateway` `valkey` и `Secret` с сертификатом для поддоменов `valkey-wildcard-tls`. Для каждого инстанса фоновая синхронизация API создаёт пространство имён `valkey-<slug>` с метками `valkey.h3llo-demo.com/instance=<slug>` и `valkey.h3llo-demo.com/user-id=<uuid>`. Внутри лежат CR `ValkeyInstance`, `Secret`, неизменяемые `ConfigMap`, прямые `Pod`, `Service`, `TCPRoute`, `SecurityPolicy`, `PodDisruptionBudget` для `ha` и `NetworkPolicy`. Созданные оператором дочерние объекты получают прямую управляющую ссылку на CR; оператор также назначает такую ссылку доставленному API объекту `Secret` после появления UID CR.
...
- селекторы Service и мягкого `TopologySpreadConstraint` включают `app.kubernetes.io/instance=<slug>`; Service дополнительно выбирает роль;
...
Только активный оператор с правом управления из `LeaseScope` запускает `ValkeyInstanceReconciler` и отдельный `ValkeyMetricsReconciler`. Для одного инстанса проходы каждого контроллера выполняются последовательно; разные инстансы могут обрабатываться параллельно. Контроллеры могут одновременно читать один инстанс, поэтому каждый из них проверяет `resourceVersion` при записи `status`.
...
| CR → объекты k8s   | оператор | `spec`, Secret, поды, ноды, шлюз и `INFO` для проверки состояния | Объекты инстанса, служебные поля Secret, `status` CR и Events                               | События и `RequeueAfter`; здоровье и операции каждую 1 с, `status.observedAt` каждые 10 с |
| CR → снимки метрик | оператор | `status.nodes`, Secret, поды, `INFO` и `pods.metrics.k8s.io`     | Только `status.metrics`                                                                     | События смены процесса и `RequeueAfter`; по умолчанию каждые 10 с                         |
...
CR связывает API и оператор. В `spec` API записывает настройки, версию пароля и поколение конфигурации. В `status` оператор сохраняет фактическое состояние, применённые версии, результаты проверки сети и последние снимки метрик текущих процессов. Паролей и хешей в CR нет.
...
В v1 образ Valkey закреплён и не меняется при активных инстансах. Оператор сохраняет `VALKEY_IMAGE` в `status.valkeyImage` до создания первого процесса. Расхождение настройки окружения с сохранённым образом даёт `RECOVERY_REQUIRED`, а замены продолжают использовать прежнее значение. Обновление Valkey относится к v2.

[Полная схема CR и смысл служебных полей](SYSTEM-DETAIL.md#cr-valkeyinstance).

### Что создаёт reconcile на один инстанс

Оператор создаёт один прямой `Pod` для `single` или три для `ha`, конфигурацию Valkey, внутренние `Service` и сетевые политики. Для публичного доступа он добавляет маршруты инстанса в общий шлюз; при включённом списке разрешённых адресов создаёт политики фильтрации по IP.

В `ha` Kubernetes старается распределить поды по разным `kubernetes.io/hostname` с `maxSkew=1`, но это предпочтение не запрещает совместное размещение. При достаточных ресурсах полный состав может работать на одной или двух нодах. Обязательные запросы ресурсов, taints и cordon по-прежнему могут оставить Pod в `Pending`. PodDisruptionBudget требует сохранить минимум два готовых пода при добровольном выселении, например обслуживании ноды. От аварии или прямого удаления Pod он не защищает, поэтому оператор сам проверяет безопасность замены.
...
Для `single` новый Valkey запускается с пустым кэшем. В `ha` оператор назначает пригодную реплику основным сервером, если удалён прежний primary, и восстанавливает потерянную копию. Возможна потеря последних записей. Если на двух оставшихся нодах хватает обязательных ресурсов, Kubernetes размещает там третий процесс и инстанс возвращается в `running` до восстановления потерянной ноды. При нехватке ресурсов третий Pod остаётся `Pending`, а доступный primary и одна синхронизированная реплика дают `degraded`.
...
Оператор сохраняет в CR целевое поколение, размер и образ до замены первого `Pod`. Изменение размера сохраняет прежний образ Valkey; обновление образа существующих инстансов относится к v2. Стадии операции сохраняются для продолжения после рестарта.
...
`ValkeyInstanceReconciler` ставит `RequeueAfter`, чтобы проверять таймауты из раздела 4 без отдельного планировщика. `ValkeyMetricsReconciler` тем же способом назначает следующий сбор через `config.MetricsInterval`. Доступ к часам идёт через интерфейс `Clock`, в тестах он подменяется.
...
| `cert-manager-webhook.valkey.h3llo-demo.com` | A   | внутренний IP рабочей ноды     |

Все записи в режиме DNS only (серое облако в Cloudflare). TLS завершается на нашей стороне; обычное проксирование Cloudflare не поддерживает TCP на нестандартном порту (часть 2). Сертификаты для `app` и `logs` получает Caddy, wildcard-сертификат для инстансов — cert-manager. Служебная запись `cert-manager-webhook` нужна только плоскости управления Kubernetes и должна совпадать с текущим `InternalIP` рабочей ноды.

При создании инстанса DNS не меняется: все `<slug>` резолвятся в один IP, инстансы различает шлюз по SNI.

Env: `VALKEY_BASE_DOMAIN` (`valkey.h3llo-demo.com` на проде, `valkey.localhost` на dev) и `VALKEY_PUBLIC_PORT`. API записывает `VALKEY_PUBLIC_PORT` в адрес клиента, а оператор использует свою настройку с тем же именем как порт listener Envoy. Обычно значения совпадают. При NAT они могут различаться: внешний порт API перенаправляется на NodePort общего listener оператора.

### Нестандартный порт

Внешний порт инстансов задаёт `VALKEY_PUBLIC_PORT` процесса API, по умолчанию `41379`. Оператор читает свою переменную с тем же именем и использует её для общего listener Envoy. Он уменьшает число обращений от массовых сканов порта 6379, но не защищает инстансы. Пароль, TLS и whitelist работают независимо от порта (часть 2). Интерфейс показывает `rediss://app:<PASSWORD>@<host>:41379` с заполненными host и port, но с заглушкой вместо пароля. Внутри кластера порт 6379.
...
Прокси Envoy настраивается объектом `EnvoyProxy`: две реплики (`spec.provider.kubernetes.envoyDeployment.replicas: 2`) с мягким `podAntiAffinity` по `kubernetes.io/hostname`. В текущем кластере обе реплики работают на одной ноде; при добавлении нод планировщик старается разнести их. Такое размещение не защищает от отказа единственной ноды. Service шлюза типа `LoadBalancer` с `externalTrafficPolicy: Local`: это первый способ сохранить IP клиента для whitelist, proxy protocol через `ClientTrafficPolicy` остаётся запасным.
...
Плоскость управления h3llo не видит кластерный `Service` обработчика допуска. Поэтому `cert-manager-webhook` работает в одном экземпляре с `hostNetwork` и портом `443`, а конфигурации допуска обращаются к `cert-manager-webhook.valkey.h3llo-demo.com`. Запись содержит внутренний адрес рабочей ноды. Процесс выпуска сверяет её с текущим `InternalIP` и останавливается при расхождении. Только этот контейнер запускается с UID 0 и `NET_BIND_SERVICE`; повышение прав запрещено, остальные возможности Linux удалены, корневая файловая система доступна только для чтения. Контроллер `cert-manager` использует системный DNS ноды через `podDnsPolicy: Default`.
...
Локальный клиент подключается к `<slug>.valkey.localhost:41379` и `<slug>-ro.valkey.localhost:41379` через NodePort шлюза в k3s. Bootstrap устанавливает wildcard-сертификат mkcert; клиент проверяет его по локальному корневому сертификату. В `single` оба адреса ведут на primary, в `ha` адрес `-ro` выбирает готовую реплику.
...
Квота по умолчанию (4 vCPU, 12 GB) ограничивает расход суммой 7920 ₽ за 720 часов на пользователя. Фактический расход за календарный месяц считается по длительности: в месяце из 31 дня полный расход квоты стоит 8184 ₽.
...
| ------------------ | -------------------------------------------------------- | ------------------------------------------------------------- |
| Пользователя       | `user_quotas`                                            | 4 vCPU и 12 GB                                                |
| Managed Kubernetes | `MANAGED_K8S_CLUSTER_VCPU`, `MANAGED_K8S_CLUSTER_RAM_GB` | 16 vCPU / 64 GiB физически, 15% CPU и 10% RAM зарезервировано |

Максимальный размер инстанса и бюджет Managed Kubernetes отвечают за разные ограничения. Первый ограничивает размер одного процесса Valkey. Два параметра `MANAGED_K8S_CLUSTER_*` задают суммарные физические ресурсы кластера без описания числа и размера worker-нод. API оставляет 15% CPU и 10% RAM инфраструктуре. Для 16 vCPU / 64 GiB внутренний бюджет равен 13600m / 58982 MiB, а `/capacity` показывает целый лимит 13 vCPU / 57 GiB. Это настроенный бюджет сервиса, а не текущее значение Kubernetes `allocatable` и не показания загрузки нод. Ограничение в 32 инстанса следует из числа `listener` одного `Gateway` и не требует отдельной переменной окружения.

Использование считается как сумма `processes * greatest(vcpu, applied_vcpu)` и `processes * greatest(ram_gb, applied_ram_gb)` по всем строкам с `deleted_at is null`. CPU и RAM сравниваются независимо, `processes` равно 1 для `single` и 3 для `ha`. Фаза не исключает инстанс из расчёта: provisioning, error, unavailable и deleting занимают ресурсы. Создание резервирует квоту при принятии запроса, даже если в Kubernetes ещё нет объектов. Только подтверждённый soft delete освобождает резерв.

Ресурсные квоты и предел 32 инстанса проверяются при создании и изменении размера внутри одной транзакции под общей блокировкой `pg_advisory_xact_lock(1)` (раздел 5). Проверка одновременно учитывает суммарные CPU и RAM, но не пытается доказать физическую раскладку по нодам. Отказ не создаёт инстанс, аудит, биллинговый период или запись ключа. При изменении размера собственный резерв заменяется кандидатом без двойного учёта. Если лимит вручную снизили ниже использования, изменение допустимо по каждой размерности, когда оно не увеличивает прежний резерв.

`GET /v1/managed/valkey/capacity` одним запросом PostgreSQL возвращает личный лимит и расход, суммарный доступный после инфраструктурного резерва бюджет и расход всех владельцев, а также предел и число активных инстансов. Маршрут не обращается к Kubernetes и не берёт общую блокировку. Страница создания и окно изменения тарифа запрашивают этот снимок сразу, затем примерно раз в пять секунд. Следующий запрос начинается только после завершения предыдущего. Личная квота имеет приоритет над общей ёмкостью, а предел инстансов применяется только к созданию.

Принятый по сумме запрос может не разместиться из-за фактической формы worker-нод, уже назначенных Pod, taints или других обязательных правил Kubernetes. В таком случае Pod остаётся `Pending`, а оператор показывает неполный состав. API не обещает, что любая комбинация ниже публичных 13 vCPU / 57 GiB физически разместится.

На физическом кластере 4 vCPU / 16 GiB внутренний бюджет после резерва равен 3400m / 14745 MiB, а публичный лимит равен 3 vCPU / 14 GiB. Поэтому каталог по-прежнему показывает размер `single` 4 vCPU / 16 GiB, но API и интерфейс отвечают, что ресурсов Managed Kubernetes недостаточно.

Резерв памяти, backlog и риск OOM при полной синхронизации описаны в разделе 6.

При полностью свободной квоте пользователя 4 vCPU / 12 GB для `ha` подходят только 1 vCPU и 1, 2 или 4 GB на ноду: всего 3 vCPU и 3, 6 или 12 GB. Даже 2 vCPU на ноду требуют 6 vCPU и недоступны без повышения квоты. Мастер показывает ресурсы на ноду и суммарно на три ноды. Неподходящие размеры остаются видны, но отправить их нельзя; интерфейс различает личную квоту, общий бюджет Managed Kubernetes и предел числа баз.
...
| Worker-нода с репликой Valkey | Primary продолжает обслуживать запросы                       | После удаления ноды через ЛК замена автоматическая при наличии ресурсов на оставшихся нодах, при потере связи требуется подтверждение остановки |
...
Мягкое распределение уменьшает вероятность совместного размещения процессов `ha`, но не гарантирует устойчивость к потере ноды. На одной ноде могут оказаться два или все три процесса. Статус `running` подтверждает процессную избыточность и синхронизацию, а не число занятых хостов. Простой включает и время восстановления после обнаружения сбоя. Оператор в `v1` имеет одну реплику; её перенос и получение `Lease` могут занять минуты. После удаления ноды через ЛК перенос автоматический по принятому порядку уничтожения виртуальной машины и удаления `Node`; при существующей недоступной ноде он ждёт подтверждения остановки прежнего процесса. `StatefulSet` оператора не удаляется принудительно ради ускорения переноса. Резервная реплика оператора и автоматическое выключение нод через API h3llo остаются за рамками `v1`.
...
`POST /v1/auth/register` принимает email и пароль аккаунта не короче 8 символов и не длиннее 72 байт UTF-8 для bcrypt, хеширует с cost 12. В одной транзакции создаёт пользователя с `is_blocked=false` и квоту 4 vCPU / 12 GB. Занятый email даёт `409 CONFLICT`; гонка с `check-email` обрабатывается этой же ошибкой. Регистрация сразу выдаёт JWT. Почту не подтверждаем; риск нескольких аккаунтов записан в границах v1 (раздел 1).
...
Один параметризованный Compose используется для пяти типов изолированных окружений:
...
| operator    | 1–4 для k3s-сценариев | unit/envtest, жизненный цикл Valkey с Envoy и восстановление после потери ноды |
| integration |                     1 | общие тесты API и оператора                                                    |
| e2e         |                     3 | полный стек для локального каталога или одной CI-группы Playwright             |

Dev сохраняет Compose-проект `managed-valkey-dev`, стабильные порты и тома. Каждый тестовый запуск получает отдельные Compose-проект, сеть, том, kubeconfig и localhost-порты. API, integration и e2e используют разные базы на общем локальном PostgreSQL; тестовая очистка не удаляет сервер или dev-базу. Общая интеграция запускает один API и один оператор, а e2e добавляет один web и последовательно проверяет полный пользовательский путь через Playwright.

`just run` читает локальный `.env`, а при его отсутствии использует полный набор
значений из `.env.example`. Тестовые команды не загружают эти файлы и получают
настройки от обвязки своего запуска.
...
Образы и инструменты закрепляем конкретными версиями или контрольными суммами, `latest` не используем. Изменение комбинации проходит интеграционную проверку перед развёртыванием. Пользовательские процессы Valkey работают в прямых `Pod` с `spec.restartPolicy: Never` и не требуют `ContainerRestartRules`. Рабочий кластер `v1.34.1` нужно повторно проверить полным сценарием выпуска после обновления оператора и сценария развёртывания.
...
Полный локальный набор оператора стремимся выполнять примерно за 10 минут на
Ryzen 9 7950X (32 потока, 64 GiB RAM, обычно около 32 GiB свободно), включая
подготовку и очистку стендов при уже скачанных образах. Это ориентир, а не
жёсткий порог успешности. Более долгий запуск допустим, если дальнейшее ускорение
требует несоразмерных затрат или усложняет сопровождение. Полноту проверок и
изоляцию сценариев ради времени не сокращаем.

Параллельные сценарии оператора получают независимые k3s с минимальной
необходимой топологией: одна нода для `single` и совместно размещённого `ha`, две
для переноса `single`, разнесённых Envoy и состава `ha` 2+1, три для предпочтительного
разнесения `ha` и четыре только для сценариев с запасной нодой. Число нод включает
server; конкретный сценарий может требовать другого размещения. Unit и envtest не требуют k3s. Число одновременно работающих
стендов ограничиваем общим бюджетом ресурсов с учётом API и остальных наборов.
После завершения или отмены запуска очищаем принадлежащие ему процессы,
контейнеры, тома, сети и маршруты, включая правила имитации отказов.

### E2E восстановления после удаления ноды

Полная матрица потери worker-ноды входит в локальный `operator just test-full`
и не запускается заданием `test-operator` в CI/CD. Она проверяет восстановление
без ручной правки подов и CR, работу `single`, реплик и Envoy. При наличии
запасной ноды должен восстановиться весь состав; без неё для `ha` допустим
`degraded`. Playwright отдельно проверяет ограниченный пользовательский сценарий
потери ноды с primary на общем трёхнодовом e2e-стенде.
...
`tests/e2e` проверяет регистрацию, жизненные циклы `single` и `ha`, личную и
общую квоту, потерю primary Pod и потерю worker-ноды. Локальный запуск одним
процессом Playwright последовательно выполняет каталог на общей PostgreSQL,
трёхнодовом k3s, одном API, одном операторе, одном web и двух репликах Envoy.
В CI группы жизненных циклов, квот и отказов идут параллельно на трёх отдельных
стендах, сохраняя последовательность внутри группы. Каждый сценарий использует
отдельный аккаунт и удаляет пользовательские ресурсы через личный кабинет.

В headless-режиме действия выполняются без демонстрационной задержки. Видимый
браузер и ручная production-проверка выдерживают не менее трёх секунд между
действиями пользователя; ожидания состояния, проверки и обращения к Valkey или
Kubernetes не замедляются. Теги `@prod`, `@local` и `@local-k8s` определяют
доступные окружения. Production-команда запускает только `@prod` и после успеха
не оставляет Valkey-инстансов.

### Justfile

Корневой `just run` готовит dev-базу и k3s, выполняет bootstrap и миграции, затем запускает оператор, API и Vite. `just down` останавливает dev-инфраструктуру без удаления данных. Корневые `just format`, `just lint` и `just test` обрабатывают API, оператор и web; линтинг и быстрые тесты выполняются параллельно. `just test` не создаёт k3s и рассчитан примерно на пять минут при установленных зависимостях. `just test-full` запускает полные наборы приложений и общую интеграцию API с оператором, но не Playwright. E2e запускается только отдельными `just test-e2e`, `just test-e2e-headed` и `just test-e2e-prod`.

[Команды сборки, миграций и тестов](SYSTEM-DETAIL.md#justfile).

### CI

Задание `test-operator` вызывает `operator just test`. Оно выполняет unit и
envtest без подготовки k3s и сборки тестового образа оператора, сохраняет журналы
и явно отмечает, что полная k3s-матрица не запускалась.

Каждый запрос на слияние и `push` сразу создаёт одиннадцать независимых заданий:
три проверки исходного кода, быстрые тесты API, оператора и клиентского приложения,
общую интеграцию, полный сквозной набор Playwright и три сборки образов. API
использует отдельную базу без k3s, оператор запускает модульные тесты и `envtest`,
а клиентское приложение запускает Vitest. Общая интеграция готовит
однонодовый k3s. Задание полного сквозного набора создаёт три параллельных задания матрицы для
жизненных циклов, квот и отказов; каждая получает отдельный трёхнодовый стенд.
Сборки API с миграциями, Caddy с клиентским приложением и оператора выпускают образы для
`linux/amd64` независимо от результатов проверок. Образ оператора при `push` в
`main` публикуется в `ghcr.io/rostislavdugin/managed-valkey-operator` с тегом SHA.

Итоговое задание дожидается всех одиннадцати результатов и принимает только
...
- Kubernetes выполняет `spec.restartPolicy: Never` для прямого `Pod`, а удаление рабочей ноды через ЛК сначала уничтожает виртуальную машину и только потом объект `Node`. Потеря связи сама по себе не разрешает переключение основного процесса.
...
### Почему изначальное решение на `StatefulSet` не сработало

Изначально пользовательские процессы Valkey запускались через `StatefulSet` со стратегией `OnDelete`. Он давал устойчивые имена `Pod` и поддерживал нужное число процессов. После завершения Valkey мы хотели оставить тот же `Pod` остановленным: оператор должен был сохранить `state.terminated`, подтвердить остановку конкретного процесса и только потом разрешить удаление и замену `Pod`.

Шаблон `StatefulSet` принимает только `spec.restartPolicy: Always`. Чтобы не перезапускать именно контейнер Valkey внутри такого `Pod`, мы задали контейнеру `restartPolicy: Never`. Это поле относится к экспериментальной возможности `ContainerRestartRules` и переопределяет общую политику `Pod` для отдельного контейнера.

Рабочий Kubernetes 1.34.1 принимал поле, но процессы `kubelet` на нодах его не исполняли: завершившийся контейнер Valkey запускался снова внутри прежнего `Pod`. Задать `spec.restartPolicy: Never` всему шаблону нельзя из-за ограничения `StatefulSet`, а изменить настройки управляемого кластера мы не можем. Поэтому пользовательские процессы переходят на отдельные `Pod` с обычным `spec.restartPolicy: Never`; их создание и безопасная замена становятся обязанностью оператора. Сам оператор остаётся в своём `StatefulSet`.
...
Одна реплика оператора в `StatefulSet` упрощает передачу управления, но увеличивает время восстановления при потере его ноды. Истёкший `Lease` сам по себе не разрешает второму процессу вмешиваться. После удаления ноды через ЛК Kubernetes запускает замену без ручного вмешательства по принятому порядку удаления виртуальной машины. При штатной смене оператор закрывает старые административные соединения и читает реальные роли; этот порядок описан в [алгоритмах оператора](SYSTEM-DETAIL.md#три-цикла). Автоматический активный резерв в `v1` не реализуем.
...
Клиентские соединения не зависят от API и PostgreSQL, поэтому запущенные процессы продолжают обслуживать их при отказе этих компонентов. Восстановление процессов требует оператора. Мягкое распределение Valkey и две реплики Envoy уменьшают вероятность общей потери процессов, но сами по себе не подтверждают выполнение SLA. Внешнюю проверку доступности через шлюз добавляем после v1.
...
k3s в compose даёт стенды на одной, двух и трёх нодах с тем же мягким распределением, что в рабочем окружении. В среде разработки оператор запускается на хосте и работает с Kubernetes API через отдельный kubeconfig. Bootstrap добавляет точные маршруты от хоста к Pod CIDR выбранного кластера. API и оператор используют отдельные тестовые k3s только в `just test-full`. Быстрые `just test` и задание CI `test-operator` работают без k3s; у оператора они выполняют unit/envtest.
