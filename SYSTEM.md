...
Правило: `ram_gb >= vcpu` и `ram_gb <= 16 * vcpu`. Пресеты в интерфейсе на старте: 1/1, 1/2, 1/4. `VALKEY_INSTANCE_MAX_VCPU` и `VALKEY_INSTANCE_MAX_RAM_GB` ограничивают размер одного инстанса; `GET /v1/managed/valkey/sizes` отдаёт сетку в этих пределах вместе с ценами.
