...
Рабочие ноды должны видеть друг друга и конечные точки Cilium. Перед изменением инфраструктуры выпуск проверяет, что каждый агент Cilium сообщает `N/N reachable`, где `N` равно числу нод. При разделённой межнодовой сети выпуск не запускает оператор новой ревизии и не включает фоновые процессы API.
...
| Managed Kubernetes | `MANAGED_K8S_CLUSTER_VCPU`, `MANAGED_K8S_CLUSTER_RAM_GB` | 12 vCPU / 48 GiB физически, 15% CPU и 10% RAM зарезервировано |

Максимальный размер инстанса и бюджет Managed Kubernetes отвечают за разные ограничения. Первый ограничивает размер одного процесса Valkey. Два параметра `MANAGED_K8S_CLUSTER_*` задают суммарные физические ресурсы кластера без описания числа и размера worker-нод. API оставляет 15% CPU и 10% RAM инфраструктуре. Для 12 vCPU / 48 GiB внутренний бюджет равен 10200m / 44236 MiB, а `/capacity` показывает целый лимит 10 vCPU / 43 GiB. Это настроенный бюджет сервиса, а не текущее значение Kubernetes `allocatable` и не показания загрузки нод. Ограничение в 32 инстанса следует из числа `listener` одного `Gateway` и не требует отдельной переменной окружения.
...
`docker-compose.prod.yml` задаёт физическую ёмкость кластера 12 vCPU / 48 GiB двумя суммарными параметрами. После резерва API учитывает 10200m / 44236 MiB и показывает в `/capacity` целый лимит 10 vCPU / 43 GiB.
