...
The exception is text where the exact characters carry the meaning: code and
identifiers, config, logs, command output, filenames and paths, quoted source
text. Leave those byte for byte, including inside a document you are otherwise
rewriting.
...
This rule applies to English. Russian text keeps `—` where grammar requires it
(`Москва — столица`, dialogue, ranges); scrub only the decorative asides where a
comma, colon, or period would do.
...
### 34. Слова должны иметь опору

**Words to watch:** канвас, шара, селектор, пейволл, фолбэк, бэктик, апрув, любая транслитерация английского слова при живом русском эквиваленте.
**Problem:** в тексте стоит существительное без опоры: в коде под таким именем ничего нет, обычным словом языка оно тоже не является. Читатель не может ни посмотреть определение, ни догадаться по смыслу. Так пишут изнутри своей головы, где референт очевиден, потому что автор только что видел его в редакторе.

Каждое существительное — одно из трёх: идентификатор из кода (в обратных кавычках), обычное слово языка, или термин, объяснённый здесь же при первом употреблении. Слова между этими категориями запрещены: они выглядят терминами, но искать их негде.

Проверка перед сдачей текста: по каждому существительному ответить «где читатель посмотрит, что это». Нет ответа — заменить на идентификатор из кода или на обычное слово.

**Англицизм — только если он уже общепринят или это имя из кода.** Транслитерация английского слова при живом русском эквиваленте — мусор. Слово из интерфейса или кода пишется как в коде и в обратных кавычках, а не переводится на слух.
**Before:**
> Блок выглядит успешным в ответе чата и на канвасе, но пейволл не работает.
**After:**
> Блок отображается и в ответе чата, и на странице в редакторе, но платный доступ не работает.

**Называй наблюдаемое, а не механизм.** Пиши, что видно снаружи, а не через какой внутренний компонент это прошло. Если компонент существенен, назови его точно и в обратных кавычках, а не намёком.
**Before:**
> Значение вне списка селектора.
**After:**
> Значение, которого нет в списке допустимых для этого свойства.

**Пример — это объект, поле и симптом, все три.** Пример пишется так, чтобы по нему можно было найти место в коде или воспроизвести случай.
**Before:**
> Кнопка без текста.
**After:**
> Кнопка `invite_friend_button` с пустой подписью.

Правило действует на всю прозу, включая ответы в чате, а не только на документы в репозитории.
