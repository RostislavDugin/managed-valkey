...
const ROLE_LABELS: Record<ValkeyNodeRole, string> = {
  primary: 'primary',
  replica: 'реплика',
  ...
  useEffect(() => {
    ...
    const load = async () => {
      ...
      try {
        ...
        const newNodeIds = nextResponse.nodes
          .map((node) => node.id)
          .filter((id) => !knownNodeIdsRef.current.has(id));
        for (const node of nextResponse.nodes) {
          knownNodeIdsRef.current.add(node.id);
        }
        setVisibleNodeIds((current) => [...current, ...newNodeIds]);
